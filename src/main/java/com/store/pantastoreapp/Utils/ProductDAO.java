...
public class ProductDAO {
    ...
    private Product mapRow(ResultSet rs) throws SQLException {
        ...
        // Will throw a clearer message if exp_date missing
        try {
            p.setExp(rs.getDate("exp_date"));
        } catch (SQLException e) {
            throw new SQLException("Column 'exp_date' not found in query result. Verify your products table schema.", e);
        }
        ...
        p.setTimestamp(rs.getTimestamp("created_at"));
        return p;
    }

    public void insert(Product product) throws SQLException {
        String sql = "INSERT INTO products (name, price, cost, quantity, exp_date, cat_name, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DBUtil.getDataSource().getConnection();
            ...
            stmt.setTimestamp(7, product.getTimestamp());
            stmt.executeUpdate();
        }
    }
}
